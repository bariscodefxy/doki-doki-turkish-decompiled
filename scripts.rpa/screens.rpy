...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("Geçmiş Kaydı") action ShowMenu('history')
            textbutton _("İleri Sar") action Skip()
            textbutton _("Oto-Oku") action Preference("auto-forward", "toggle")
            textbutton _("Kaydet") action ShowMenu('save')
            textbutton _("Devam Et") action ShowMenu('load')
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Lütfen isminizi giriniz", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("Yeni Oyun") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Lütfen isminizi giriniz", ok_action=Function(FinishEnterName)))

            else:

                textbutton _("Geçmiş Kaydı") action [ShowMenu("history"), SensitiveIf(renpy.get_screen("history") == None)]
            ...
            if _in_replay:
                ...
                textbutton _("Yeniden Oynatmayı Bitir") action EndReplay(confirm=True)
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Yardım") action [Help("KILAVUZ.html"), Show(screen="dialog", message="Yardım dosyası tarayıcınızda açıldı.", ok_action=Hide("dialog"))]
...
screen about():
    ...
    use game_menu(_("Hakkında"), scroll="viewport"):
        ...
        vbox:
            ...
            label "[config.name!t]"
            text _("Sürüm [config.version!t]\n")


            if gui.about:
                text "[gui.about!t]\n"

            text _("{a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only] ile yapılmıştır.\n\n[renpy.license!t]")
...
init python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="Dosya hatası: \"characters/sayori.chr\"\n\nDosya kayıp veya bozuk.",
                ok_action=Show(screen="dialog", message="Kayıt dosyası bozuk. Yeni bir oyuna başlanıyor.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="Kaydetmenin bir anlamı yok artık.\nMerak etme, bir yere gitmiyorum.", ok_action=Hide("dialog"))
        else:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%d %B %Y, %A, %H:%M"), empty=_("boş yuva")):
        ...
        vbox:
            ...
            hbox:
                ...
                if config.developer:
                    vbox:
                        style_prefix "radio"
                        label _("Geri Sarma Konumu")
                        textbutton _("Devre Dışı Bırak") action Preference("rollback side", "disable")
                ...
                vbox:
                    style_prefix "check"
                    label _("İleri Sar")
                    textbutton _("Okunmamış Metni") action Preference("skip", "toggle")
                    ...
                    if config.has_sound:
                        ...
                        hbox:
                            ...
                            if config.sample_sound:
                                textbutton _("Test Et") action Play("sound", config.sample_sound)


                    if config.has_voice:
                        label _("Konuşma Seviyesi")

                        hbox:
                            bar value Preference("voice volume")

                            if config.sample_voice:
                                textbutton _("Test Et") action Play("voice", config.sample_voice)
...
screen name_input(message, ok_action):
    ...
    frame:
        ...
        input default "" value VariableInputValue("player") length 12 allow "ABCÇDEFGĞHIİJKLMNOÖPQRSŞTUÜVWXYZabcçdefgğhıijklmnoöpqrsştuüvwxyz"
        ...
        hbox:
            ...
            textbutton _("Tamam") action ok_action
