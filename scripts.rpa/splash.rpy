init python:
    menu_trans_time = 1
    splash_message_default = "Bu oyun çocuklar veya kolay\nrahatsız olanlar için uygun değildir."
    splash_messages = [
    "Sen benim gün ışığımsın,\nYegâne gün ışığım",
    "Seni özledim.",
    "Oyna benimle",
    "Sadece bir oyun.",
    "Bu oyun çocuklar veya kolay\nrahatsız olanlar için uygun değildir?",
    "sdfasdklfgsdfgsgoinrfoenlvbd",
    "null",
    "Çocukları cehenneme yolladım",
    "PM bunun için öldü.",
    "Bu sadece bir bakıma azıcık senin suçun.",
    "Bu oyun çocuklar veya kolay\nparçaya ayrılanlar için uygun değildir.",
    "Monika'nın karakter dosyasını yedeklemeyi unutma."
    ]
...
label splashscreen:
    ...
    if not firstrun:
        if persistent.first_run and (config.version == persistent.oldversion or persistent.autoload == "postcredits_loop"):
            ...
            scene black
            menu:
                "Eski bir kayıt dosyası bulundu. Kayıt dosyanızı silip baştan başlamak ister misiniz?"
                "Evet, kayıt dosyamı sil.":
                    "Kayıt dosyası siliniyor...{nw}"
                    python:
                        delete_all_saves()
                        renpy.loadsave.location.unlink_persistent()
                        renpy.persistent.should_save_persistent = False
                        renpy.utter_restart()
                "Hayır, kaldığım yerden devam et.":
    ...
    if not persistent.first_run:
        ...
        with Dissolve(1.0)
        pause 1.0
        "Bu oyun çocuklar veya kolay rahatsız olanlar için uygun değildir."
        "Anksiyete bozukluğu olan veya depresyon hastası olan kişilerin bu oyunu oynamaları onları kötü etkileyebilir. Uyarılar için lütfen ana menüdeki Yardım butonuna başvurunuz."
        menu:
            "Doki Doki Edebiyat Kulübü’nü oynayarak en az 13 yaşında olduğunuzu onaylar, yüksek derecede rahatsızlık verici içeriğe maruz kalacağınızı kabul etmiş olursunuz."
            "Kabul ediyorum.":
                pass
    ...
    if s_kill_early:
        ...
        show noise:
            alpha 0.1
        with Dissolve(1.0)
        show expression Text("Şimdi herkes mutlu olabilir.", style="sayori_text"):
            xalign 0.8
            yalign 0.5
            alpha 0.0
            600
...
label after_load:
    ...
    if persistent.yuri_kill > 0 and persistent.autoload == "yuri_kill_2":
        ...
        if persistent.playthrough == 0 and not persistent.first_load and not config.developer:
            ...
            call screen dialog("İpucu: Önceden okumuş olduğunuz metni “İleri Sar”\nbutonuna tıklayarak hızlıca geçebilirsiniz.", ok_action=Return())
    return
...
label readonly:
    scene black
    "Oyun, oyunu salt okunur bir konumdan açmaya çalıştığınız için başlatılamıyor."
    "Lütfen DDEK uygulamasını masaüstüne veya ulaşılabilir başka bir yere kopyalayıp tekrar deneyin."
    $ renpy.quit()
    return
